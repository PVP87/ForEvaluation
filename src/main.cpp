...
int main()
{
	float conf_threshold = 0.5f; //порог доверия
	float nms_threshold = 0.5f; //минимальный порог доверия

	// Проверка правильности указанного входного файла
	...
	// Создание очередей для отправки изображения на дисплей и на детектирование
	// Создаются умные указатели image_queue и detection_queue, которые управляют объектами типа 
	// MessageQueue<cv::Mat>.
	// Эти объекты создается динамически с использованием оператора new.
	// Класс MessageQueue предназначен для хранения объектов типа cv::Mat(изображений OpenCV).
	...
	input.thread_for_read(); //прочитали видео и создали очередь кадров в указателе detection_queue
